...
if run_solver_from_scratch
    ...
    solver = FIBSolver(max_iterations=2, verbose=true)
    # solver = QMDPSolver(max_iterations=2, belres=1e-3, verbose=true)
    ...
    # save the policy
    save("fib_policy-1-1-50.jld2", "policy", policy)
else
    # loading policy
    println("loading policy")
    policy = load("qmdp_policy-1-1-50.jld2", "policy")
end
